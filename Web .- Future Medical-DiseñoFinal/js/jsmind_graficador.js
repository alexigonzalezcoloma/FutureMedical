...
function open_file(){
    ...
    if(files.length > 0){
        ...
        var format = file_data.name.split('.');
        
        if(format[format.length-1].toLowerCase() != "json" && format[format.length-1].toLowerCase() != "xml"){
          prompt_info("Extensión inválida. El editor soporta JSON y XML");
          flag = false;
        }
        ...
        jsMind.util.file.read(file_data,function(jsmind_data, jsmind_name){
            ...
            if(!!mind){
                ...
                prompt_info('No es posible abrir el archivo como un mindmap');
            }
        });
        }
    }
...
function add_node(){
    ...
    if(!selected_node){prompt_info('Debes seleccionar un nodo');return;}
